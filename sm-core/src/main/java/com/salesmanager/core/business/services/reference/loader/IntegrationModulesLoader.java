...
public class IntegrationModulesLoader {
	...
	private IntegrationModuleSource moduleSource;
	private IntegrationModuleParser moduleParser;

	public IntegrationModulesLoader() {
		this.moduleSource = new DefaultIntegrationModuleSource();
		this.moduleParser = new DefaultIntegrationModuleParser();
	}

	public IntegrationModulesLoader(IntegrationModuleSource moduleSource, IntegrationModuleParser moduleParser) {
		this.moduleSource = moduleSource;
		this.moduleParser = moduleParser;
	}

	public List<IntegrationModule> loadIntegrationModules(String jsonFilePath) throws Exception {
		String jsonContent = moduleSource.loadRawJson(jsonFilePath);
		return moduleParser.parseModules(jsonContent);
	}
	...
	private class DefaultIntegrationModuleSource implements IntegrationModuleSource {
		@Override
		public String loadRawJson(String identifier) throws ServiceException {
			try {
				InputStream in = IntegrationModulesLoader.this.getClass().getClassLoader()
						.getResourceAsStream(identifier);
				if (in == null) {
					throw new ServiceException("File not found: " + identifier);
				}
	...
	private class DefaultIntegrationModuleParser implements IntegrationModuleParser {
		@Override
		public List<IntegrationModule> parseModules(String jsonContent) throws ServiceException {
